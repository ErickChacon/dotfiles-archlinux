...
if [ $internal = "true" ]; then
    ...
    main_ix=1; sec_ix=2; main_sc=1; sec_sc=1.5;
else
    # main_ix=2; sec_ix=1; main_sc=1.5; sec_sc=1.2;
    main_ix=2; sec_ix=1; main_sc=1; sec_sc=1.2;
fi
if [ $only_one = "true" ]; then
    sec_sc=0;
fi

echo "main:" $main_ix "with scale" $main_sc
echo "secondary:" $sec_ix "with scale" $sec_sc
...
echo "main:" $main_mon $main_res $main_pan
